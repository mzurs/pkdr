...
const retainVerification = async (
  ...
  if (addressResult.result) {
    const res: RetainVerificationResult = {
      message: "",
      result: false,
    };
    ...
    try {
      const tx = await connectedWallet.retainVerification(
        addressResult.message
      );
      await listenForTransactionMine(tx, provider);

      res.message = `Verification of ${userName} Retained successfully`;
      res.result = true;
      return res;
    } catch (error) {
    ...
    return addressResult as RetainVerificationResult;
  }
};

export default retainVerification;
